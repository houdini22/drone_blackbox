/****************************************************************************
** Meta object code from reading C++ file 'sendingarduino.h'
**
** Created by: The Qt Meta Object Compiler version 63 (Qt 4.8.7)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../sendingarduino.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'sendingarduino.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 63
#error "This file was generated using the moc from 4.8.7. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_SendingArduino[] = {

 // content:
       6,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       1,       // signalCount

 // signals: signature, parameters, type, tag, flags
      16,   15,   15,   15, 0x05,

 // slots: signature, parameters, type, tag, flags
      57,   55,   15,   15, 0x0a,
      95,   55,   15,   15, 0x0a,
     134,   15,   15,   15, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_SendingArduino[] = {
    "SendingArduino\0\0signalSendingDataChanged(SendingData*)\0"
    ",\0slotArduinoStatusChanged(int,QString)\0"
    "slotArduinoConnected(bool,SerialPort*)\0"
    "slotArduinoReset()\0"
};

void SendingArduino::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Q_ASSERT(staticMetaObject.cast(_o));
        SendingArduino *_t = static_cast<SendingArduino *>(_o);
        switch (_id) {
        case 0: _t->signalSendingDataChanged((*reinterpret_cast< SendingData*(*)>(_a[1]))); break;
        case 1: _t->slotArduinoStatusChanged((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< QString(*)>(_a[2]))); break;
        case 2: _t->slotArduinoConnected((*reinterpret_cast< bool(*)>(_a[1])),(*reinterpret_cast< SerialPort*(*)>(_a[2]))); break;
        case 3: _t->slotArduinoReset(); break;
        default: ;
        }
    }
}

const QMetaObjectExtraData SendingArduino::staticMetaObjectExtraData = {
    0,  qt_static_metacall 
};

const QMetaObject SendingArduino::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_SendingArduino,
      qt_meta_data_SendingArduino, &staticMetaObjectExtraData }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &SendingArduino::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *SendingArduino::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *SendingArduino::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_SendingArduino))
        return static_cast<void*>(const_cast< SendingArduino*>(this));
    if (!strcmp(_clname, "SendingInterface"))
        return static_cast< SendingInterface*>(const_cast< SendingArduino*>(this));
    return QObject::qt_metacast(_clname);
}

int SendingArduino::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    }
    return _id;
}

// SIGNAL 0
void SendingArduino::signalSendingDataChanged(SendingData * _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}
QT_END_MOC_NAMESPACE
